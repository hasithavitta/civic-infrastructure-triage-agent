...
def test_run_intake_text_only(mock_gemini, mock_geocoding):
    ...
    mock_gemini.set_response('{"issue_type": "pothole", "description": "Large pothole.", "address_or_landmark": "Main Street", "has_discrepancy": false}')
...
def test_pii_redaction_before_model_call(mock_gemini, mock_geocoding):
    mock_gemini.set_response('{"issue_type": "pothole", "description": "redacted info", "address_or_landmark": "Main St", "has_discrepancy": false}')
    
    report = run_intake(raw_text="Call me at 123-456-7890 or email test@example.com.")
...
def test_geocoding_called_with_address(mock_gemini, mock_geocoding):
    mock_gemini.set_response('{"issue_type": "pothole", "description": "Pothole", "address_or_landmark": "Charminar", "has_discrepancy": false}')
...
def test_multimodal_called_when_image_present(mock_gemini, mock_geocoding):
    mock_gemini.set_response('{"issue_type": "pothole", "description": "Pothole in image", "address_or_landmark": "Main Street", "has_discrepancy": false}')
    
    report = run_intake(raw_text="Pothole", image_filename="photo.jpg", image_bytes=b"fake-bytes", image_mime_type="image/jpeg")
